...
class SyncData:
    def google_available_lots_list(self)->list:
        resp = GoogleSheetClient().get_sheet(Config.sheet_key,Config.avail_parking_lots_sheet_id)[ColumnName.all_parking_places].values.tolist()
        return  resp
    ...
    def copy_reserve_from_google_to_df(self):
        ...
        for index, row in google_df.iterrows():
            Querry().insert_reserve(row[ColumnName.user_name], row[ColumnName.surname], row[ColumnName.parking_lots], row[ColumnName.reservation_time])
    
    def copy_user_data_from_db_to_google(self):
        GoogleSheetClient().set_sheet(Config.sheet_key,Config.user_data_sheet_id, get_query_to_pandas(Querry.select_all_user_data))
