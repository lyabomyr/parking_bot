...
def remove_reserve(message):
    ...
    try:
        if str(user_id) in df[ColumnName.user_id].values.tolist():
            ...
            park_bot.reply_to(message, f'You removed reserve from parking lot: *{df.parking_lots[df.user_id == str(user_id)].values.tolist()}*', parse_mode="Markdown")
...
def enter_parking_lots_step(message):
    try:
        ...
        mysql_execute(Querry().insert_reserve(user_id, login_name, surname[0], parking_number, FuncHandBot().time_now()))
        park_bot.reply_to(message, f'Added new booking for parking lot: {parking_number}\nby USER: {login_name},\nwith  SURNAME: {surname[0]} \nIN: {FuncHandBot().time_now()}',reply_markup=markup)
        FuncHandBot().status_notification(Config.chat_id)
        mysql_execute(Querry.insert_google_flag)      
    except Exception as e:
        print(e)
